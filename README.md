...
- 全ユーザーの音声をミックスした単一のWAVファイルとして保存（48kHz / stereo）
- 全員退出時に自動で切断・保存
...
プロジェクトルートに `config.md` を作成すると、議事録生成時のプロンプトを変更できます。未作成の場合はデフォルトのプロンプトが使用されます。
...
recordings/
  2026-02-28/
    ...
    transcript.txt         # Whisper文字起こし（OPENAI_API_KEY設定時）
    議事録_2026-02-28.md    # GPT-4o議事録（OPENAI_API_KEY設定時）
  2026-02-28_2/
    recording.wav
    ...
```
